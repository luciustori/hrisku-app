...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Validasi NIK & KTP unique
    $checkQuery = "SELECT COUNT(*) as count FROM employees WHERE nik = :nik OR ktp_number = :ktp";
    $checkStmt = $db->prepare($checkQuery);
    $checkStmt->execute(['nik' => $nik, 'ktp' => $ktp_number]);
    
    if ($checkStmt->fetch(PDO::FETCH_ASSOC)['count'] > 0) {
        $error = 'NIK atau Nomor KTP sudah terdaftar';
    } else {
        try {
            $query = "INSERT INTO employees 
                      (nik, full_name, ktp_number, birth_place, birth_date, gender, address, 
                       phone, email, department_id, position_id, branch_id, employment_status, 
                       join_date, contract_end_date, is_active) 
                      VALUES 
                      (:nik, :full_name, :ktp_number, :birth_place, :birth_date, :gender, :address,
                       :phone, :email, :department_id, :position_id, :branch_id, :employment_status,
                       :join_date, :contract_end_date, 1)";
            ...
            redirect(BASE_URL . 'modules/kepegawaian/index.php?success=add');
        ...
        <div class="card-body">
            <form method="POST" action="">
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label>Telepon</label>
                        <input type="tel" name="phone" class="form-control" placeholder="08xx">
                    </div>
                    <div class="form-group">
                        <label>Email</label>
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <small class="text-muted">Kosongkan jika karyawan tetap</small>
                    </div>
                </div>
...
<script>
// Ajax untuk load positions berdasarkan department
